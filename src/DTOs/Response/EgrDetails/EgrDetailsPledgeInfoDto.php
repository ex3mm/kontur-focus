<?php

declare(strict_types=1);

namespace Ex3mm\KonturFocus\DTOs\Response\EgrDetails;

/**
 * DTO сгенерирован из docs/endpoints/egrDetails/schema.json
 * @phpstan-consistent-constructor
 */
final readonly class EgrDetailsPledgeInfoDto
{
    public function __construct(
        /** Срок обременения или порядок определения срока */
        public ?string $duration = null,
        /** Залогодержатель — юрлицо */
        public ?EgrDetailsDefinitionPledgeInfoPledgeeULDto $pledgeeUL = null,
        /** Залогодержатель — физлицо */
        public ?EgrDetailsDefinitionPledgeInfoPledgeeFLDto $pledgeeFL = null,
    ) {
    }
}
